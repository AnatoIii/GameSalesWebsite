...
namespace GamesServicesIntegrationTests
{
	public class GamesPricesControllerTests : IClassFixture<WebApplicationFactory<Startup>>
    {
        ...
        [Fact]
        public async Task GetGamesByFilter_When_Get4HavingFSortedAscendingByDiscount_Then_SecondRequestHasLessDiscount()
        {
            var httpResponse1 = await _client.GetAsync($"api/gamesPrices/filtered?from=0&CountPerPage=4&GameName=F&SortType=discount&AscendingOrder=true");
            var stringResponse1 = await httpResponse1.Content.ReadAsStringAsync();
            var gamesResponse1 = JsonConvert.DeserializeObject<IEnumerable<GameDTO>>(stringResponse1);

            //here we should get games count
            var gamesCount = 123; //gamesResponse.Count - 5;
            var httpResponse2 = await _client.GetAsync($"api/gamesPrices/filtered?from={gamesCount}&CountPerPage=4&GameName=F&SortType=discount&AscendingOrder=true");
            var stringResponse2 = await httpResponse2.Content.ReadAsStringAsync();
            var gamesResponse2 = JsonConvert.DeserializeObject<IEnumerable<GameDTO>>(stringResponse2);
        ...
        public async Task GetGamesByFilter_When_GetByCategory_Then_OnlySuchGamesExists()
        {
            ...
            var httpResponse = await _client.GetAsync($"api/gamesPrices/filtered?from=0&CountPerPage=25&Platforms=[1,3]");
            var stringResponse = await httpResponse.Content.ReadAsStringAsync();
            var gamesResponse = JsonConvert.DeserializeObject<IEnumerable<GameDTO>>(stringResponse);
